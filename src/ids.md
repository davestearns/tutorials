...
The IDs generated by this look like `3bcgkj480l0w8n36` by default. They are relatively short (16 characters), case-insensitive, and safe to embed in URLs. 

By default, this uses a 32 bit random value, which is enough to generate a hundred or so IDs per-nanosecond with an extremely low probability of collision. If you need more than that, increase the `num_random_bits` parameter. For example, 40 random bits would let you generate about ten thousand IDs per-nanosecond with a similarly low probability of collision, while increasing the encoded length by only one character.
