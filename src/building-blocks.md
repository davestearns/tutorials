...
If your API is only used sporadically, you can often save money by turning them into so-called "serverless" functions behind an API gateway. For example, on AWS you could use their [API Gateway](https://docs.aws.amazon.com/apigateway) product to expose a set of APIs, and configure each to trigger a specific [AWS Lambda function](https://docs.aws.amazon.com/lambda) when requested. 

Since AWS Lambda functions are executed asynchronously via a queue, the API Gateway essentially publishes a trigger event for your Lambda, and waits for the function to publish its response event. The Gateway then turns that response event into a proper HTTP response message.

This asynchronous invocation obviously requires more work and incurs more latency than a traditional API server that receives and processes the HTTP request directly. And since Lambda functions run on AWS-managed servers that are also running everyone else's Lambda functions, their execution speed is typically slower than a dedicated HTTP server running on its own EC2 instance. Lambda functions also suffer from the occasional **cold start problem** where the server running your function may need to download a new version and initialize it before the function can be executed. You might be able to avoid this using their [SnapStart](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html) feature, but it currently has a lot of [limitations](https://docs.aws.amazon.com/lambda/latest/dg/snapstart.html#snapstart-runtimes).

But the good news is that you are only charged for the CPU and resources used by your Lambda function while it was _actually running_. If your API gets only a few requests per-day, you only pay for those few short invocations, not the rest of the time when the HTTP server was processing other requests for other people.

The [economics](https://www.bbva.com/en/innovation/economics-of-serverless/) of serverless functions, combined with their higher latency and cold start problem, imply that serverless functions can be a good choice for APIs that are used infrequently or sporadically. But if you are expecting many requests a second, at a more or less constant rate, and consistent performance really matters, then a continuously-running server is typically a better option.
